...
- `include_global=true`、`env=dev`、输出目录 `ninebot-project/webresources/shared/js/`
...
rm -f ninebot-project/webresources/shared/js/XRM.Options.global.js
# 本轮不再包含某实体时，删除该实体的旧文件
rm -f ninebot-project/webresources/shared/js/XRM.Options.<旧实体>.js
```

### Step 3: 语法校验
对生成的每个文件跑 `node --check`：
```bash
for f in ninebot-project/webresources/shared/js/XRM.Options*.js; do node --check "$f" && echo "OK $f"; done
```
任一失败必须修复后再继续。
...
## 备注
- 认证走 client-credentials 静默流程（凭据在 `ninebot-project/config/environments.yaml` + `.env`），无需交互登录。
