...
在 Windows 环境读取包含中文的 Excel 时，终端输出可能显示乱码。**数据本身是正确的**，问题仅在于终端显示编码。

**调试方法**：将 Excel 内容写入 UTF-8 编码的文件，然后用 Read 工具查看。
...
with open('temp.txt', 'w', encoding='utf-8') as f:
    for row in range(1, 20):
        f.write(str(ws.cell(row, 1).value) + '\n')
"
```

然后用 Read 工具读取 `temp.txt` 即可看到正确中文。
