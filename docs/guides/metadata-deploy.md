...
| Picklist **默认值** | `Column.default_value` → Dataverse 的 **`DefaultFormValue`**（不是 Boolean 专用的 `DefaultValue`），创建与补丁两条路径都携带；`-1` 表示「无默认值」，本地 `None` 不发送该键。`bool` 默认值会被**告警并跳过**（`False` 会变成选项值 0）。`plan` 的差异明细是 `fields=['DefaultFormValue']`。详见 ADR-017。⚠️ 多态 `/Attributes` **会**返回该键，故无需 typed fetch（与 ADR-010 的 OptionSet 情形相反）。 |
...
| 表定义漂移校正（reverse） | 本地 `metadata_py/tables/<t>.py` 与环境不一致时，**整表 deploy 会误建大量字段/关系**。判据：`pp plan <t>` 出现非预期 `would_create`。校正三步：①备份原定义；②`pp reverse <t> --env dev -o <tmp>.py` 先导出到**临时文件** diff（勿直接覆盖，reverse 会整文件重写）；③确认后覆盖，再 `pp plan <t>` 校验 `would_create=0`。reverse 产出是 **full snapshot**（含 `CreatedOn` 等标准字段，`deploy` 自动跳过 `would_skip_standard`）；2026-09-09 起正确保留 `optionset_name`（此前会静默降级全局选项集为本地内联，见 ADR-010 addendum）。 |
...
| App SiteMap 实体菜单（Phase 10/ADR-015） | `pp sitemap add-entity <entity> --app <app> --area <A> --group <G>`：定位 app-aware sitemap（`sitemap.sitemapnameunique == appmodule.uniquename`，appmodule 本身无 `sitemapxml` 列）→ 实体已有 SubArea 则 `skipped_unchanged`（幂等，lint 对重复报 error）→ 否则**写前备份**原始 XML 到 `docs/env_backup/sitemap_{unique}.{ts}.bak.xml` → `PATCH sitemaps({id}) {sitemapxml}`（新 SubArea 样式克隆自既有实体条目）→ 发布（定向 `PublishXml(<sitemaps>)` 400 → 回退 `PublishAllXml`）→ 回读验证。sitemap 是解决方案组件 **code 62**：已在解决方案内（如 `new_entity930`）则随其 transport，勿再 add-component。`remove-entity` 同语义（全局或按 area/group 范围）。**ADR-015 addendum（2026-09-20）**：加 `--create-group` 可在组不存在时自动新建（Id=`group_{uuid8}`，标题 1033+2052）；`_entity_display` 优先 `2052`，不带 `--title` 时菜单显示**中文**；`SiteGroup` 必须带 `Id` 属性（XSD 要求）。 |
